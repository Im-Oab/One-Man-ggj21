use tetra::graphics::{Color, GeometryBuilder, ShapeStyle};
use tetra::math::Vec2;
use tetra::Context;

use crate::gameplay::bullet_pool::{Bullet, BulletType};
...
impl BulletType for ConstantVelocityBulletType {
    ...
    fn setup(&mut self, bullet: &mut Bullet, image_assets: &ImageAssets) {
        ...
        if let Some(animation) = image_assets.get_animation_object(idle_animation_name) {
