...
impl BossEnemyType {
    ...
    fn do_action(enemy: &mut Enemy, player: Option<&Player>) {
        if enemy.state == 1 && enemy.weapon_tick == 0 {
...

            {
                ...
                play_sound_nodes.insert(
                    String::from("boss_spawn"),
                    (String::from("./resources/sfx/boss_spawn.mp3"), 0.6),
                );
            }

            {
                let mut play_sound_nodes = crate::PLAY_SOUND_NODES.lock().unwrap();
                play_sound_nodes.insert(
                    String::from("boss_enter"),
                    (String::from("./resources/sfx/boss_enter.mp3"), 0.8),
                );
            }
...
impl EnemyType for BossEnemyType {
    ...
    fn init(&mut self, enemy: &mut Enemy, image_assets: &ImageAssets) {
...

        {
            ...
            play_sound_nodes.insert(
                String::from("boss_enter"),
                (String::from("./resources/sfx/boss_enter.mp3"), 0.7),
            );
        }

        if let Some(animation) = image_assets.get_animation_object("enemy-boss-idle") {
    ...
    fn update(&self, enemy: &mut Enemy, player: Option<&Player>, _image_assets: &ImageAssets) {
    ...
    fn die(&self, enemy: &mut Enemy) {
...

        {
            let mut play_sound_nodes = crate::PLAY_SOUND_NODES.lock().unwrap();
            play_sound_nodes.insert(
                String::from("boss_explode"),
                (String::from("./resources/sfx/boss_explode.mp3"), 0.8),
            );
        }
    }
